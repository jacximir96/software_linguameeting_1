<h2 class="accordion-header" id="heading{{$section->id}}" name="anchor-accordion-{{$section->id}}">
    <button class="accordion-button {{ $viewData->sectionToExpand($section->id) ? '' : 'collapsed'}}" type="button" data-bs-toggle="collapse"
        ...
        @else
            <span class="text-danger ms-4 small" title="{{$statusSection->completed()}} sessions completed out of {{$statusSection->totalSessions()}}">
                {{$statusSection->completed()}} of {{$statusSection->totalSessions()}} assigned guides.
            </span>
        @endif
    </button>
</h2>
<div id="collapse{{$section->id}}" class="accordion-collapse collapse  {{ $viewData->sectionToExpand($section->id) ? 'show' : ''}}"
     aria-labelledby="heading{{$section->id}}"
     data-bs-parent="#accordionSection">
    <div class="accordion-body p-2 py-md-2 px-md-3 body-{{$section->id}}">
