        ...
        <div class="card-body">
            ...
            @foreach($viewData->availability()->hours() as $availabilityTimeHour )
                ...
                <div class="row mt-4">
                    ...
                    @if ($availabilityTimeHour->timeHour()->isLastMinute($viewData->timeHourSelected()->date()))
                        <div class="row">
                            <div class="col-12">

                                <span class="d-block text-corporate-danger mt-2">
                                    Sessions must be scheduled 12 hours in advance. Or you can try searching for a last minute session:
                                </span>

                                <span class="d-block text-corporate-danger mt-2">

                                    <a href="{{route('get.student.session.last_minute.use', [ $enrollment->hashId(),
                                                                                              $sessionOrder->get(),
                                                                                              $viewData->timeHourSelected()->date()->toDateString(),
                                                                                              $availabilityTimeHour->timeHour()->hashId(),
                                                                                              $request->coach ?? ''])}}"
                                       class="d-inline-block bg-corporate-color text-white p-1 border rounded small">
                                        Last Minute Booking
                                    </a>
                                </span>
                        </div>
