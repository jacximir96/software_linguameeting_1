...
class AssignMakeUpByInstructorAction
{
    ...
    public function handle(AssignMakeupToCourseByInstructorRequest $request, Course $course, User $allocator): Course
    {

        $course->load('enrollment', 'enrollment.user');

        $makeupType = $this->makeupTypeRepository->obtainBySlug(MakeupType::SLUG_INSTRUCTOR);
