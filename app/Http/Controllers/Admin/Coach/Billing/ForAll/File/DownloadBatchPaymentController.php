...
class DownloadBatchPaymentController extends Controller
{
...
;
        ...
        $now = Carbon::now();
        $filename = 'reports_coach_batch_payment_'.$now->format('Y-m-d_H_i_s');

        return Excel::download(new BatchPaymentExport($viewData, $linguaMoney), $filename.'.csv', \Maatwebsite\Excel\Excel::CSV);
    }
}
